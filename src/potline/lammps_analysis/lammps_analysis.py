...
def run_properties_simulation(out_path: str, 
    ...
    with open(SUBMIT_SCRIPT_TEMPLATE_PATH, 'r') as file_template:
        ...
        with open(simulation_script_out_path, 'w') as file_out:
            file_out.write(simulation_script_content)

    subprocess.run(['bash', simulation_script_out_path])